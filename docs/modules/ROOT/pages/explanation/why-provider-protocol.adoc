...
NOTE: These are Bolt-side provider choices. NAMS manages extraction and embeddings server-side. See xref:explanation/backends.adoc[Bolt and NAMS]. This page describes `neo4j-agent-memory` 0.6.0; check the version you have installed with `pip show neo4j-agent-memory`.
...
. Passes `llm=None` through unchanged.
