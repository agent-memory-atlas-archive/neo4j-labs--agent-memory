...
the write. The user's message
is saved once per turn, not once per step of a tool-calling loop; the
assistant's answer is saved after the model finishes, and is skipped when a
step only produced tool calls with no answer text. In tools mode, the model
...
`createNams`/`createNamsMemoryTools` in tools mode runs a real
entities-and-relationships extraction over that memory. `createNamsProvider`
