...
image::diagrams/provenance-chain-graph-auditability.svg[Provenance chain: graph traversal and vector similarity both trace agent decisions back to source messages and entities,width=100%,link=self]
...
image::diagrams/graph-provenance-tracing-decisions-to-sources.svg[Graph provenance: a single Cypher traversal from a credit decision through shared entities back to the original KYC findings,width=100%,link=self]
