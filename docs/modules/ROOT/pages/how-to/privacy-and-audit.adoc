...
* A connected Bolt `client`, with `settings` configured as in xref:how-to/messages.adoc[Store, retrieve, and summarize messages].
...
* xref:how-to/multi-tenancy.adoc[Associate memory with users and select scoped reads] — the `:User` node that audit edges attach to.
