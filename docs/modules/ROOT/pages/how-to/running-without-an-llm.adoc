...
[NOTE]
====
This configuration uses the `bolt` backend with Aura. On the hosted memory service (NAMS) extraction and embeddings run server-side, so there is no client-side LLM or embedding provider to opt out of — see xref:how-to/use-nams.adoc[Connect a Python application to NAMS].
====
...
* xref:how-to/use-nams.adoc[Connect a Python application to NAMS] — the hosted backend, where extraction and embeddings run server-side and there is no client-side LLM or embedding provider to opt out of.
* xref:tutorials/first-agent-memory.adoc[Build your first memory context] — the Aura connection setup used above.
