...
NAMS. See xref:explanation/backends.adoc[Bolt and NAMS backends].
====
...
`MemorySettings.embedding` accepts a provider string or an explicit Provider instance. xref:how-to/bring-your-own-model.adoc#_three_ways_to_wire_a_provider[Bring your own model — Three ways to wire a provider] walks through both forms for `llm=`; framework pass-through (pattern C) applies to `llm=` only. The embedding field works the same way:
...
See xref:how-to/integrations/google-cloud.adoc#vertex-ai-embedding-models[the Vertex AI model table] for the supported and retired model IDs and the `task_type` values.
...
[source,bash]
----
pip install 'neo4j-agent-memory[cli,mcp,sentence-transformers,openai]==0.6.0'
----
