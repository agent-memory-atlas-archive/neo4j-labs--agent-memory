...
[source,bash]
----
npm install @neo4j-labs/nams-ai-provider@0.3.0 @neo4j-labs/agent-memory@0.5.0 \
...
`createNamsProvider`'s stream wrapper awaits the assistant-turn write as part
...
visible success signal, use the
xref:how-to/typescript/edge-runtime.adoc[application-owned write pattern] and
pass `persistInteractions: false` to `createNamsProvider` so the response is
not stored twice.
...
shipping: a green suite here means the route, the provider wiring and the
