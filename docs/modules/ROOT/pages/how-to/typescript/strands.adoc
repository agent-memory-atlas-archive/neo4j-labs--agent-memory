...
into batches of 100 (the `bulkAddMessages` limit) and dedupes retried batches
...
// Neo4jConversationManager matches ConversationManager's public surface but
// does not extend Strands' abstract class, so strict TypeScript needs a cast.
const agent = new Agent({
  sessionManager,
  conversationManager: conversationManager as unknown as ConversationManager,
  model,
  tools,
});
...
asserts each of our classes still matches the Strands interface it implements
(for `Neo4jConversationManager`, the `name`, `reduce` and `initAgent` members of
the abstract `ConversationManager`), so an `npm install` that reshapes one of
them fails a test rather than drifting silently. We will not break the integration API across minor releases of
