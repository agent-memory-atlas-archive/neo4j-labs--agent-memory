...
* The Python SDK plus the extra for your configured embedding provider, for example `pip install 'neo4j-agent-memory[openai]==0.6.0' 'httpx>=0.27'`, which provides `client.schema.adopt_existing_graph`. Keep `httpx` in the command: release 0.6.0 imports it when connecting over Bolt but declares it only in the `nams` extra.
...
`search_entities()` does not find adopted nodes until you backfill their
embeddings. The
https://github.com/neo4j-labs/agent-memory/blob/main/examples/existing-graph/retrieve.py[`retrieve.py` script in the existing graph example]
shows a backfill that skips nodes with non-UUID ids.
...
* xref:explanation/poleo-model.adoc[The POLE+O data model] — the default ontology you're either keeping or overriding with `SchemaModel.CUSTOM`.
