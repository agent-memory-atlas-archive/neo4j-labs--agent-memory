...
Replace legacy `EmbeddingConfig` and `LLMConfig` objects with the provider API in `neo4j-agent-memory` 0.6.0.
...
* In 0.6.0, `MemorySettings` still accepts legacy `EmbeddingConfig` and `LLMConfig` objects. Each explicit legacy object emits a `DeprecationWarning`, not a `FutureWarning`.
* The warning text names a planned removal in v0.5.0, but both types are still present in 0.6.0. Treat the warning as a migration notice.
...
Install both providers used above: `pip install 'neo4j-agent-memory[anthropic,openai]==0.6.0' 'httpx>=0.27'`. Set `ANTHROPIC_API_KEY` and `OPENAI_API_KEY` for the LLM and embeddings respectively.
...
* xref:reference/configuration.adoc[Configuration reference] — the canonical `MemorySettings` reference.
