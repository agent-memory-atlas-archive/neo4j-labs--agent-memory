...
* The Python SDK with the extra for the new embedding provider, for example `pip install 'neo4j-agent-memory[openai]==0.6.0' 'httpx>=0.27'`. Keep `httpx` in the command: release 0.6.0 imports it when connecting over Bolt but declares it only in the `nams` extra. This runbook uses public Bolt graph/schema APIs; it is not a hosted workspace migration tool.

== 1. Inspect the existing indexes and choose a migration path

anchor:_the_error[]anchor:_decision_rebuild_or_revert[]anchor:_option_1_rebuild_from_scratch_lose_existing_embeddings[]anchor:_option_2_revert_to_the_previous_embedding_model[]
...
| `ReasoningStep` | `embedding` | Nonempty thought, action, and observation values prefixed with `++Thought: ++`, `++Action: ++`, and `++Observation: ++` (each followed by a space), joined with a space
|===
...
anchor:_verifying_the_migration[]anchor:_what_the_validator_actually_checks[]
