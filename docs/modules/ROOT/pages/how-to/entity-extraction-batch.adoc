...
Use Python 3.10 or newer and a repository checkout. From its root, activate a virtual environment and install the required extra:
...
The maintained program `docs/modules/ROOT/examples/extraction_recipes.py` supplies its own imports, source texts, and event-loop entry point. It downloads the GLiNER model on first inference; allow disk space and network access for the download. These commands do not connect to a database or require an LLM API key.
...
[source,bash]
----
python docs/modules/ROOT/examples/extraction_recipes.py batch
----
...
[source,bash]
----
python docs/modules/ROOT/examples/extraction_recipes.py streaming
----
