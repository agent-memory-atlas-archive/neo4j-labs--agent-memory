...
ships configuration templates, scripts and a `WALKTHROUGH.md` for testing persistence across quitting the editor. The walkthrough is not a live-service certification.
====

== Prerequisites

* This SDK checkout with `uv sync --extra nams --extra mcp` completed. The current factory helpers are absent from the selected PyPI 0.5.0 wheel.
...
The maintained example defines its full `DECISIONS` transcript and handles the client lifecycle. From the repository root:

[source,bash]
----
uv run python examples/claude-code-team-memory/seed_workspace.py --dry-run
uv run python examples/claude-code-team-memory/seed_workspace.py
----
...
This is a project-scoped template. Configure the hosted or self-hosted entry you selected; do not assume both address the same storage. Confirm environment substitution in your installed host before using `$\{VAR}` placeholders.
...
GUI hosts may have a different environment from your shell. Use the installed host's supported secret mechanism and verify the executable path. The example also includes an MCP bundle; inspect its manifest and build instructions before installing it.
...
[source,bash]
----
uv run python examples/claude-code-team-memory/doctor.py --configs-only    # offline: no key, no network
uv run python examples/claude-code-team-memory/doctor.py --conversation-id RETURNED_ID
----
...
the local Bolt core/extended counts can be checked against this checkout. This does not enumerate an authenticated hosted server. Read
