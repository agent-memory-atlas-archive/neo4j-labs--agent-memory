...
* Install the extras for each selected provider and configure its credentials. Quote package extras in shell commands, for example `pip install 'neo4j-agent-memory[openai]==0.6.0' 'httpx>=0.27'`. Keep `httpx` in the command: release 0.6.0 imports it when connecting over Bolt but declares it only in the `nams` extra.
...
* **Framework pass-through** — wrap an already-configured `ChatOpenAI`, `ChatAnthropic`, etc. with the matching `llm_provider_from_<framework>` helper. Pass-through helpers exist for every supported integration; see xref:how-to/migrate-to-providers.adoc#_pattern_5_pass_through_a_framework_native_model[Migrate to pluggable providers — Pattern 5] for the full list.
...
[source,bash]
----
pip install 'neo4j-agent-memory[cli,mcp,anthropic,openai]==0.6.0' 'httpx>=0.27'
----
