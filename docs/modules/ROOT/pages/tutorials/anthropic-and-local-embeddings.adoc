...
. One or more `Entity candidate:` lines, one for each entity that extraction created or updated during this run, then `Verified: context assembly returned text`.
...
The program reads the entities back with an exact graph query rather than `search_entities()`. In 0.6.0, entities created by automatic extraction are stored without an embedding, so a semantic entity search does not return them. Messages are embedded, which the next step checks.
