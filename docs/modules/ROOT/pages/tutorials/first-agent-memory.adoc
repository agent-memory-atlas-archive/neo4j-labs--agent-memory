...
anchor:_prerequisites[]anchor:_what_youll_learn[]anchor:_time_required[]
...
We also install `httpx`, which release 0.6.0 imports when connecting over Bolt but declares only in the `nams` extra.
...
anchor:_step_2_set_up_neo4j[]anchor:_option_a_docker_recommended_for_learning[]anchor:_option_b_neo4j_aura_free[]
...
anchor:_step_4_create_your_first_memory_client[]anchor:_step_5_add_entity_extraction[]
...
anchor:_step_7_view_your_context_graph_in_neo4j_browser[]anchor:_view_all_nodes[]anchor:_view_messages_and_their_entities[]anchor:_view_user_preferences[]anchor:_view_the_full_context_graph[]
...
anchor:_troubleshooting[]anchor:_connection_refused[]anchor:_authentication_failed[]anchor:_gliner_download_slow[]
