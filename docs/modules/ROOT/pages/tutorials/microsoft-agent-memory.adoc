...
[NOTE]
====
icon:flask[] *Preview.* This adapter is a preview integration. It targets the Microsoft Agent Framework 1.x GA line (`agent-framework-core>=1.13,<2`); adapter and framework APIs can change between releases. Pin the versions you tested and re-check the integration guide before upgrading a deployment.
====
...
[source,bash]
----
python -m pip install 'neo4j-agent-memory[microsoft-agent,openai]==0.6.0' 'httpx>=0.27'
