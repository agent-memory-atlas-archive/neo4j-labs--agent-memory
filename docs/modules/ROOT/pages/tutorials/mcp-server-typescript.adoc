...
* macOS, Git, Node.js 22.9 or later with npm, and a POSIX shell. This lesson is
...
cd my-memory-mcp
npm init -y
npm install @neo4j-labs/agent-memory@0.5.0 @modelcontextprotocol/sdk@^1.30 zod@^4
...
`@neo4j-labs/agent-memory@0.5.0` package from npm; the checkout supplies only
...
xref:reference/backend-capabilities.adoc#hosted-cleanup[Hosted cleanup and retained resources]).
Applications that decide how long such records are kept can use the reasoning
methods listed in xref:reference/typescript-api.adoc[the API reference].
