...
Add an LLM extractor stage. Despite the name, the stage is not a fallback: the pipeline that `build()` returns runs every stage on every call and merges the results with the selected merge strategy, so the LLM is called for every text even when spaCy or GLiNER find entities. The builder has no early-stop option. For true fallback behaviour, construct xref:reference/extractor-pipeline.adoc[`ExtractionPipeline`] directly with `stop_on_success=True` or `MergeStrategy.FIRST_SUCCESS`.
