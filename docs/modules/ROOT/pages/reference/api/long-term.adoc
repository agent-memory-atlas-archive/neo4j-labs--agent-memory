...
| `applies_to` | Optional list of xref:reference/api/reasoning.adoc#_entityref[EntityRef] describing the entities this preference scopes to. Each ref is materialized as a `(:Preference)-[:APPLIES_TO]->(:Entity)` edge so `get_preferences_for(user, applies_to=...)` queries can find scoped preferences efficiently.
|===
