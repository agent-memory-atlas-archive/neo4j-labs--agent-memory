...
| Parsed active document and binding metadata from the authoritative response `version`; never inferred from the latest revision.
...
For `get_active()` / `getActive()`, a supplied response `version` identifies the
bound version even when a newer revision exists. Python exposes its
`version_id`, `revision` and `validation_mode`; TypeScript exposes `versionId`,
`revision` and `validationMode`. Legacy document-only responses, or responses
with a missing/null `version`, leave optional binding metadata absent. A
consumer that must restore a binding must stop before mutation when that
metadata is unavailable. Partial, malformed or contradictory supplied version
metadata is an error; neither client silently substitutes the latest revision.
