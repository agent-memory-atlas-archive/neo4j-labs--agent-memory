...
[NOTE]
====
This map describes the released `@neo4j-labs/agent-memory@0.5.0` package. Where
the repository source already behaves differently, the affected entry says so.
The SDK shares a memory model with the Python SDK, but method availability
differs by backend and language. The normal TypeScript application uses hosted REST; the optional bridge transport
...
|Wire memory into a Vercel AI SDK `generateText` / `streamText` call. Implements specification v4 (`ai` 7.x). Requires the optional `@ai-sdk/provider` peer for its exported types. See xref:how-to/typescript/vercel-ai.adoc[Vercel AI SDK middleware].
...
|AWS Strands Agents SDK (JS). Requires the optional `@strands-agents/sdk` peer. See xref:how-to/typescript/strands.adoc[Strands].
...
**Related package**: xref:how-to/typescript/nams-ai-provider.adoc[`@neo4j-labs/nams-ai-provider`]
...
xref:reference/nams-ai-provider.adoc[its API reference].
